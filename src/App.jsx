        ...
        <nav style={{flex:1,padding:"0 9px",display:"flex",flexDirection:"column",gap:1}}>
          {NAV.map(n=>(
            <button key={n.id} className="nb" onClick={()=>setScreen(n.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 11px",borderRadius:8,border:"none",background:screen===n.id?"rgba(255,255,255,0.18)":"transparent",color:screen===n.id?"white":"rgba(255,255,255,0.52)",fontWeight:screen===n.id?700:400,fontSize:12,fontFamily:"'Sora',sans-serif",cursor:"pointer",transition:"all 0.16s",borderLeft:screen===n.id?"3px solid rgba(255,255,255,0.7)":"3px solid transparent"}}>
              ...
              {n.id==="expenses"&&todayExp>spendLimit&&<span style={{marginLeft:"auto",background:"#ef4444",color:"white",borderRadius:99,width:16,height:16,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800}}>!</span>}
            </button>
          ))}
        </nav>
      ...
      <div style={{marginLeft:220,minHeight:"100vh",display:"flex",flexDirection:"column",width:"calc(100% - 220px)",boxSizing:"border-box"}}>
        <div style={{padding:"22px 24px 0",position:"relative",zIndex:10}}>
          ...
          {screen==="scenarios"&&(
            <div className="sc">
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:9,marginBottom:14}}>
                {SCENARIOS_DEF.map(s=>(
                  <button key={s.id} onClick={()=>setScenario(s.id===scenario?null:s.id)} style={{padding:"13px 8px",borderRadius:12,border:`2px solid ${scenario===s.id?s.color:bdr}`,background:scenario===s.id?s.bg:card,cursor:"pointer",transition:"all 0.22s",display:"flex",flexDirection:"column",alignItems:"center",gap:6,boxShadow:scenario===s.id?`0 0 16px ${s.color}33`:"none",fontFamily:"'Sora',sans-serif"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"270px 1fr",gap:11,animation:"fadeUp 0.3s ease"}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                      ...
                      <div style={{background:card,borderRadius:13,padding:16,border:`1px solid ${bdr}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                        <Lbl style={{marginBottom:5}}>Wellness Score</Lbl>
                        <Ring score={Math.max(0,Math.min(100,wellness+impact.sc))} size={82} dark={pro}/>
                        <div style={{fontSize:10,fontWeight:800,color:impact.sc<0?"#ef4444":"#10b981",marginTop:3}}>{impact.sc>0?"+":""}{impact.sc} pts</div>
                      </div>
          ...
          {screen==="expenses"&&(
            <div className="sc">
              <div style={{marginBottom:13,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                <div><Lbl style={{color:"rgba(255,255,255,0.65)"}}>Daily Spending Tracker</Lbl><h1 style={{fontSize:23,fontWeight:800,color:"white"}}>Expenses</h1></div>
                <button onClick={()=>setAddExpOpen(true)} style={{padding:"7px 14px",background:"rgba(255,255,255,0.18)",border:"1px solid rgba(255,255,255,0.28)",color:"white",borderRadius:9,fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>+ Add Expense</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:10,marginBottom:12}}>
                {[
                  {l:"Today's Spending", v:fc(todayExp,cur), c:todayExp>spendLimit?"#ef4444":"#10b981", sub:todayExp>spendLimit?`⚠ ${fc(todayExp-spendLimit,cur)} over limit`:`${fc(spendLimit-todayExp,cur)} remaining`},
                  {l:"Daily Limit",      v:fc(spendLimit,cur), c:accentPrimary, sub:"Tap to edit"},
                  {l:"This Week",        v:fc(SPENDING_HIST.reduce((s,d)=>s+d.amount,0),cur,true), c:txt, sub:"7-day total"},
                  {l:"Avg Per Day",      v:fc(Math.round(SPENDING_HIST.reduce((s,d)=>s+d.amount,0)/7),cur), c:txt, sub:"Daily average"},
                ].map((x,i)=>(
                  <div key={i} className="hov" onClick={i===1?()=>setEditLimit(true):undefined} style={{background:card,borderRadius:13,padding:16,border:`1px solid ${i===0&&todayExp>spendLimit?"#fca5a5":bdr}`,cursor:i===1?"pointer":"default",transition:"all .3s"}}>
                    <Lbl>{x.l}</Lbl>
                    <div style={{fontSize:18,fontWeight:800,color:x.c,marginBottom:3}}>{x.v}</div>
                    <div style={{fontSize:9,fontWeight:700,color:i===0&&todayExp>spendLimit?"#ef4444":sub}}>{x.sub}</div>
                  </div>
                ))}
              </div>
              {todayExp>spendLimit&&(
                <div style={{background:"#fff1f0",border:"1px solid #fca5a5",borderRadius:11,padding:"11px 16px",marginBottom:11,display:"flex",gap:10,alignItems:"center"}}>
                  <span style={{fontSize:18}}>🚨</span>
                  <div style={{flex:1}}><strong style={{fontSize:12,color:"#dc2626"}}>Daily Spending Limit Exceeded!</strong><div style={{fontSize:10,color:"#7f1d1d",marginTop:2}}>You've spent <strong>{fc(todayExp,cur)}</strong> today — <strong>{fc(todayExp-spendLimit,cur)}</strong> over your {fc(spendLimit,cur)} limit.</div></div>
                  <button onClick={()=>setEditLimit(true)} style={{padding:"6px 12px",background:"#dc2626",color:"white",border:"none",borderRadius:7,fontSize:9,fontWeight:700,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>Edit Limit</button>
                </div>
              )}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:11}}>
                <div className="hov" style={{background:card,borderRadius:14,padding:20,border:`1px solid ${bdr}`,transition:"all .3s"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                    <div><div style={{fontSize:12,fontWeight:800,color:txt}}>7-Day Spending Pattern</div><div style={{fontSize:9,fontWeight:600,color:sub}}>Red bars = over limit</div></div>
                    <Tag color={accentPrimary}>Limit: {fc(spendLimit,cur)}</Tag>
                  </div>
                  <ExpBar data={SPENDING_HIST} limit={spendLimit} cur={cur}/>
                </div>
                <div className="hov" style={{background:card,borderRadius:14,padding:20,border:`1px solid ${bdr}`,transition:"all .3s",maxHeight:280,overflowY:"auto"}}>
                  <div style={{fontSize:12,fontWeight:800,color:txt,marginBottom:12}}>Today's Transactions</div>
                  {expenses.filter(e=>e.date==="today").length===0
                    ?<div style={{textAlign:"center",padding:"24px",color:sub,fontSize:11}}>No transactions today yet</div>
                    :expenses.filter(e=>e.date==="today").map((e,i)=>(
                      <div key={e.id} style={{display:"flex",alignItems:"center",gap:9,padding:"8px 0",borderBottom:`1px solid ${bdr}`}}>
                        <div style={{width:8,height:8,borderRadius:"50%",background:expCatColors[e.category]||"#94a3b8",flexShrink:0}}/>
                        <div style={{flex:1}}><div style={{fontSize:11,fontWeight:800,color:txt}}>{e.name}</div><div style={{fontSize:9,fontWeight:600,color:sub}}>{e.category} · {e.time}</div></div>
                        <strong style={{fontSize:12,color:txt}}>{cur.symbol}{(e.amount*cur.rate).toFixed(2)}</strong>
                      </div>
                    ))
                  }
                </div>
              </div>
              <div className="hov" style={{background:card,borderRadius:14,padding:20,border:`1px solid ${bdr}`,marginTop:11,transition:"all .3s"}}>
                <div style={{fontSize:12,fontWeight:800,color:txt,marginBottom:12}}>Today's Spending by Category</div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                  {Object.entries(expenses.filter(e=>e.date==="today").reduce((acc,e)=>({...acc,[e.category]:(acc[e.category]||0)+e.amount}),{})).map(([cat,amt],i)=>(
                    <div key={i} style={{background:"#f8fafc",borderRadius:10,padding:"11px 12px",border:`1px solid ${bdr}`,borderLeft:`3px solid ${expCatColors[cat]||"#94a3b8"}`}}>
                      <div style={{fontSize:9,fontWeight:800,color:expCatColors[cat]||"#94a3b8",marginBottom:4}}>{cat}</div>
                      <strong style={{fontSize:14,color:txt}}>{cur.symbol}{(amt*cur.rate).toFixed(2)}</strong>
                    </div>
                  ))}
                </div>
              </div>
              {editLimit&&(
                <Modal title="Set Daily Spending Limit" subtitle="You'll get an alert when you exceed this amount" onClose={()=>setEditLimit(false)}>
                  <Inp label={`Daily Limit (${cur.code})`} value={Math.round(spendLimit*cur.rate)} onChange={v=>setSpendLimit(parseFloat(v)/cur.rate||100)} type="number"/>
                  <div style={{display:"flex",gap:8,marginTop:6}}>
                    <button onClick={()=>setEditLimit(false)} style={{flex:1,padding:"9px",background:"transparent",border:`1px solid ${bdr}`,borderRadius:9,color:sub,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>Cancel</button>
                    <button onClick={()=>setEditLimit(false)} style={{flex:2,padding:"9px",background:`linear-gradient(135deg,${accentPrimary},#6366f1)`,color:"white",border:"none",borderRadius:9,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>Save Limit</button>
                  </div>
                </Modal>
              )}
              {addExpOpen&&(
                <Modal title="Add Expense" subtitle="Log a new transaction" onClose={()=>setAddExpOpen(false)}>
                  <Inp label="Description" value={newExpense.name} onChange={v=>setNewExpense(e=>({...e,name:v}))} placeholder="e.g. Grab Ride, Lunch..."/>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                    <div style={{marginBottom:11}}>
                      <Lbl>Category</Lbl>
                      <select value={newExpense.category} onChange={e=>setNewExpense(ex=>({...ex,category:e.target.value}))} style={{width:"100%",padding:"9px 11px",borderRadius:9,border:"1px solid #e2e8f0",background:"#f8fafc",color:"#0f172a",fontSize:12,fontFamily:"'Sora',sans-serif",outline:"none"}}>
                        {EXP_CATS.map(c=><option key={c}>{c}</option>)}
                      </select>
                    </div>
                    <Inp label={`Amount (${cur.code})`} value={newExpense.amount} onChange={v=>setNewExpense(e=>({...e,amount:v}))} type="number" placeholder="0.00"/>
                  </div>
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={()=>setAddExpOpen(false)} style={{flex:1,padding:"9px",background:"transparent",border:`1px solid ${bdr}`,borderRadius:9,color:sub,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>Cancel</button>
                    <button onClick={addExpense} style={{flex:2,padding:"9px",background:`linear-gradient(135deg,${accentPrimary},#6366f1)`,color:"white",border:"none",borderRadius:9,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>Add Expense</button>
                  </div>
                </Modal>
              )}
            </div>
